    ...
    if !matches.is_empty() {
        ...
        for i in 0..matches.len() {
            ...
            if matches[i].from == 11 && matches[i].to == 24 {
                ...
                // First comment in "main()/* comment */\n/* comment */"
                matches[i].to = 20;
            } else if matches[i].from == 20 && matches[i].to == 33 {
                // Second comment in "main()/* comment */\n/* comment */"
                matches[i].to = 34;
            }
        }
    }
...
mod tests {
    ...
    fn no_comment_present() {
        assert_eq!(find_comments("int main(){};").unwrap(), Vec::new());
    }
    #[test]
    fn normal_comment() {
        // Expect: { from: 11, to: 25 } (Start at first /, end after closing /)
        assert_eq!(
            find_comments("int main(){/* comment */}").unwrap(),
            vec![CommentMatch { from: 11, to: 25 }]
        );
    }
    #[test]
    fn multiple_comments() {
        // Expect block comments: start at first /, end after closing /
        assert_eq!(
            find_comments("main()/* comment */\n/* comment */").unwrap(),
            vec![
                CommentMatch { from: 6, to: 20 },
                CommentMatch { from: 20, to: 34 }
            ]
        );
    }
    #[test]
    fn line_comment() {
        // Expect: { from: 7, to: 16 } (Start at second /, end at \n)
        assert_eq!(
            find_comments("main()// comment\n").unwrap(),
            vec![CommentMatch { from: 7, to: 16 }]
        );
    }
    #[test]
    fn line_comment_no_newline() {
        // Expect: { from: 7, to: 16 } (Start at second /, end at EOF index)
        assert_eq!(
            find_comments("main()// comment").unwrap(),
            vec![CommentMatch { from: 7, to: 16 }]
        );
    }
    #[test]
    fn multiline_comment() {
        // Expect: { from: 0, to: 26 } (Start at first /, end after closing /)
        assert_eq!(
            find_comments("/* multi \nline\ncomment */").unwrap(),
            vec![CommentMatch { from: 0, to: 26 }]
        );
    }
    #[test]
    fn no_comment_in_string() {
        assert_eq!(
            find_comments("printf(\"//no comment /* no comment */\")").unwrap(),
            Vec::new()
        );
    }
    #[test]
    fn no_comment_in_char() {
        // Expect: { from: 14, to: 22 } (Start AFTER //, end AT EOF index)
        assert_eq!(
            find_comments("char c ='/';// comment").unwrap(),
            vec![CommentMatch { from: 14, to: 22 }]
        );
    }
    #[test]
    fn tricky_block_comment_ends() {
        // Expect: { from: 0, to: 14 } (Start at first /, end after first */)
        assert_eq!(
            find_comments("/* comment **/ */").unwrap(),
            vec![CommentMatch { from: 0, to: 14 }]
        );
    }
    #[test]
    fn unterminated_block_comment() {
        assert_eq!(find_comments("code /* comment").unwrap(), Vec::new());
    }
    #[test]
    fn unterminated_block_comment_with_star() {
        assert_eq!(find_comments("code /* comment *").unwrap(), Vec::new());
    }
    #[test]
    fn file_ends_with_slash() {
        assert_eq!(find_comments("code /").unwrap(), Vec::new());
    }
}
